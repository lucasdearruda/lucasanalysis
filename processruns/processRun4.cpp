...
int processRun4(int nrun = 317) {
    ...
    for(int tel = 1; tel <= 4; tel++) {
        for(int particle = 1; particle <= 3; particle++) {
            new TCanvas(Form("c%dtel%d",particle,tel),Form("c%dtel%d",particle,tel),100+50*tel+10*particle,100+50*tel+10*particle,800,600);
            gt[tel][particle]->Draw();
            cout<<"tel = "<<tel<<" particle = "<<particle<< " eval = "<<gt[tel][particle]->Eval(6)<<endl;
        }
    }
        ...
        for(int telN = 1; telN <= 4; telN++){
            ...
            if(particle){
                ...
                if(particle<=3 && energy>Emin[telN][particle]){
                    //cout<<"tel = "<<telN<<" particle = "<<particle<< " eval = "<<de1+de2<<endl;
    ...
    timer.Print();
    cout<<"Compiled from processRun4.cpp"<<endl;
    return 0;
