...
//
//Script done specially for the reversed (T8@20deg) runs
...
//I CHANGED THIS FOR THE REVERSED POSITION
// https://docs.google.com/spreadsheets/d/1F_ebqCgmyMwVjdBosSgu1X9jGyOU7WLDEL8LTEfx0EQ/edit?gid=1032030769#gid=1032030769
	double tgamma;
...
int main(int argc, char* argv[]) {
	...
	if (argc != 2) {
    	std::cerr << "Usage: " << argv[0] << " nrun" << std::endl;
    ...
    const char * outputFileName  = Form("/home/e802/Analysis/LucasAnalysis/reducedData/%03d.root",nrun);
    ...
    TBranch * tofBranchNC  = newtree->Branch("tofnNC", &tofnNC, "tofnNC/F");
    TBranch * angleBranch  = newtree->Branch("ang", &ang, "ang/F");
    TBranch * EnBranch  = newtree->Branch("ENN", &ENN, "ENN/F");
    TBranch * EnBranchNC  = newtree->Branch("ENNnc", &ENNnc, "ENNnc/F");
    ...
    for (Long64_t entry = 0; entry < nEntries; entry++) {
        ...
        for(int telN = 1; telN <= 4; telN++){
            ...
            if(particle){
                ...
                if(particle<=3 && energy>Emin[telN][particle]){
        ...
        // Print the progress
        std::cout << "Progress: " << 100.0*entry/(nEntries) << "%\r";
        std::cout.flush();
    ...
    //some information to add
        ...
        TNamed *dist_Medleytarget_T1_in_mm = new TNamed("dist_Medleytarget_T1(mm)",to_string(L[1]).c_str());;
        TNamed *dist_Medleytarget_T2_in_mm= new TNamed("dist_Medleytarget_T2(mm)",to_string(L[2]).c_str());;
        TNamed *dist_Medleytarget_T3_in_mm= new TNamed("dist_Medleytarget_T3(mm)",to_string(L[3]).c_str());;
        TNamed *dist_Medleytarget_T4_in_mm= new TNamed("dist_Medleytarget_T4(mm)",to_string(L[4]).c_str());;
    ...
    //---ooOOOoo---.---ooOOOoo---.---ooOOOoo---.---ooOOOoo---.---ooOOOoo---.---ooOOOoo---.---ooOOOoo---.---ooOOOoo---.
    TH1D * hp[5],* hpNC[5],*hd[5],*hdNC[5],*ht[5],*hh[5],*ha[5];
    ...
    for(int telN = 1; telN <= 4; telN++){
        ...
        hpn[telN] = new TH2D(Form("protonsE_EnNC%d",telN),Form("Eprotons:ENNnc_Tel %d", telN), 1000,0,50, 1000,0,50);

        hdn[telN] = new TH2D(Form("deuteronsE_En%d",telN),Form("Edeuterons:ENN_Tel%d", telN), 1000,0,50, 1000,0,50);
        hdn[telN] = new TH2D(Form("deuteronsE_EnNC%d",telN),Form("Edeuterons:ENNnc_Tel%d", telN), 1000,0,50, 1000,0,50);
        ...
        newtree->Draw(Form("energy>>protonsE%d",telN),Form("PID==1&&ang==%.1f",80+telN*20.0));
        newtree->Draw(Form("energy>>deuteronsE%d",telN),Form("PID==2&&ang==%.1f",80+telN*20.0));
        newtree->Draw(Form("energy>>tritonsE%d",telN),Form("PID==3&&ang==%.1f",80+telN*20.0));
        newtree->Draw(Form("energy>>heliumE%d",telN),Form("PID==4&&ang==%.1f",80+telN*20.0));
        newtree->Draw(Form("energy>>alphasE%d",telN),Form("PID==5&&ang==%.1f",80+telN*20.0));

        newtree->Draw(Form("energy:ENN>>protonsE_En%d",telN),Form("PID==1&&ang==%.1f",80+telN*20.0));
        newtree->Draw(Form("energy:ENNnc>>protonsE_EnNC%d",telN),Form("PID==1&&ang==%.1f",80+telN*20.0));
    
        newtree->Draw(Form("energy:ENN>>deuteronsE_En%d",telN),Form("PID==2&&ang==%.1f",80+telN*20.0));
        newtree->Draw(Form("energy:ENNnc>>deuteronsE_EnNC%d",telN),Form("PID==2&&ang==%.1f",80+telN*20.0));

        newtree->Draw(Form("energy:ENN>>tritonsE_En%d",telN),Form("PID==3&&ang==%.1f",80+telN*20.0));
        newtree->Draw(Form("energy:ENN>>heliumE_En%d",telN),Form("PID==4&&ang==%.1f",80+telN*20.0));
        newtree->Draw(Form("energy:ENN>>alphasE_En%d",telN),Form("PID==5&&ang==%.1f",80+telN*20.0));
