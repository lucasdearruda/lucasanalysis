//prod_DXX.cpp, version 1.2025-08-05.0
...
void prod_DDX(bool plotIt = true) {
    ...
    std::vector<float> angles =  {20.0, 140.0};
...
TNamed *processing_info= new TNamed("File produced by prod_DDX.cpp:",Form("version1.2025-08-05.0 in %s",cur_time.c_str()));
...
for (const auto& bin : energy_bins) {
    ...
    for(float angle : angles){
        ...
        for(char particle : particles){
            ...
            hist = newE(hist, hname+"newE", false, thicknessFirstTel(angle/20)); // Apply the newE function to the histogram
    ...
    cout<<"prod_DXX.cpp, version 1.2025-08-05.0"<<endl;
