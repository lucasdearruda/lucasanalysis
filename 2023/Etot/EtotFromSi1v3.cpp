// Script that fix the peak, and try to fix the neutron energy as well 
//
...
void EtotFromSi1v3(string runfile = "/mnt/medley/LucasAnalysis/2023/reducedv61/370.root", bool savehistos = false, string savingname = "/mnt/medley/LucasAnalysis/2023/applyTTC/protons370.root"){
...
for(Long64_t l = 0;l<nEntries;l++){
    ...
    if(pid==1 && ang == 20.0 && si1>0 && si2>0&& csi>0){
        ...
        h2energies->Fill(si1+si2+csi,newE);
...
if(savehistos){
    ...
    cout<<"Saving histograms into "<<savingname<<"..."<<endl;
    TFile *f = new TFile(savingname.c_str(),"RECREATE");
    horig->Write();
    hnew->Write();
    h2->Write();
    h2energies->Write();
