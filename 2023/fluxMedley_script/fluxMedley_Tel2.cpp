...
void fluxMedley_Tel2(int telN = 2 ){
...
trpol->Draw("ENN>>protons_pol", Form("PID==1 && ang ==%f && ENN>2.5 ",telN*20.0),"colz");
...
trpol->Draw("tofn>>htof_pol", Form("PID==1 && ang ==%f && ENN>2.0 ",telN*20.0));
trcarbon->Draw("tofn>>htof_carbon", Form("PID==1 && ang ==%f && ENN>2.0 ",telN*20.0),"same");
...
float transformation_factor = 4*sin(telN*20*TMath::Pi()/180);// 2*TMath::Pi()*sin(telN*20*TMath::Pi()/180);
