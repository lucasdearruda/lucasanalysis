...
void fluxMedley_comparingTransformations(int telN = 1 ){
...
//doing the same thing for the nflux_np obtained spectra
for(int b=1; b<= nflux_np->GetNbinsX(); b++){
    
    
    if(nflux_np->GetBinLowEdge(b)<= 40){
    //Get the corresponding XS: 
        xscalc = transformation_factor*xs->Eval(nflux_np->GetBinCenter(b));

        //get the bin-width from the neutrons' energy histo
        bw = nflux_np->GetBinWidth(b);

        //calculate: Φ(E) = Cte * #(E) /[dσ/dΩ(E)]
        //here I divided to binwidth to be comparible with the other results
        value = Cte*nflux_np->GetBinContent(b)/(xscalc*bw);

        //fill the value in fluxpol 
        nflux_np->SetBinContent(b,value);
    }else{
        nflux_np->SetBinContent(b,0.0);
    }   
}

TCanvas *flux_flox_tof_Canvas = new TCanvas("flux_flox_tof_Canvas","flux_flox_tof_Canvas",100,100,800,600);
nflux_np->Draw();
...
//Axis labels and drawing 
 ...
 fluxpol->SetFillColor(kCyan);
 fluxpol->SetLineColor(kCyan);
 fluxpol->Draw();


 nflux_np->SetLineWidth(2);
 nflux_np->SetFillStyle(3004);
 nflux_np->SetFillColor(kBlack);
 nflux_np->SetLineColor(kBlack);
 nflux_np->Draw("same");
...
tl->AddEntry(nflux_np,"flux from np (ENN after Tof sub)", "fl");
